...
BaseTFModel::BaseTFModel(const std::string &modelgen_path,
                         ...
                         const std::string &graph_path)
    : // BaseModel(),
      modelgen_path_(("/home/star/" + modelgen_path)), // lion::FileUtil::GetRelativeToRootPath
      pymodel_path_(("/home/star/" + pymodel_path)), // lion::FileUtil::GetRelativeToRootPath(
      graph_path_(("/home/star/" + graph_path)) { // lion::FileUtil::GetRelativeToRootPath
  tf_session_entity_ = std::unique_ptr<TfSessionEntity<float, float>>(
      new TfSessionEntity<float, float>());
  // lion_ASSERT(FileUtil::Exists(pymodel_path_));
}
...
void BaseTFModel::TFInit() {
  ...
  lion_ASSERT(tf_session_entity_->IsStatusOk());
}
...
void BaseTFModel::GenerateModel(const std::string &args_str) {
  ...
  lion_ASSERT(succ == 0);
  // lion_ASSERT(FileUtil::Exists(graph_path_));
}
}  // namespace brain
}  // namespace lion