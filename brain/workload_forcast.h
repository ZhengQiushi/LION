...
namespace brain {
     ...
     */
    lion::matrix_eig GetWorkload(std::vector<std::vector<int> > &frequnce) {
    ...
    public:
        my_predictor(int preiod, int feats, int bptt, int horizon, float val_split){
            ...
            model = std::unique_ptr<lion::brain::TimeSeriesLSTM>(new lion::brain::TimeSeriesLSTM(
                NFEATS,
                ...
                EPOCHS));
            n =  lion::brain::Normalizer(NORMALIZE);
            early_stop_patience = lion::brain::CommonWorkloadDefaults::ESTOP_PATIENCE;
            early_stop_delta = lion::brain::CommonWorkloadDefaults::ESTOP_DELTA;
            
            DCHECK(model->IsTFModel() == true);
        }

        void train(lion::matrix_eig& data){
        ...
        lion::matrix_eig predict(lion::matrix_eig& data){
            n.Fit(data);
        ...
        lion::brain::Normalizer& get_normalizer(){
            return n;
        }
    private:
    std::unique_ptr<lion::brain::TimeSeriesLSTM> model;
    lion::brain::Normalizer n;
